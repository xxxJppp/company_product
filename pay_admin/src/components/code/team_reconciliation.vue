	...
	<div>
		...
		<form class="am-form am-g">
			...
			<div class="common_title clearfix">
					<span v-show="isSelectSub" class="_info fl" style="line-height: 32px; margin-right: 20px;" @click="select_data">
				...
				<!-- 搜索按钮 -->
				<a @click="select_data" href="javascript:void(0)" class="fl ml15 select_btn"><i class="el-icon-search"></i> &nbsp;搜索</a>
			...
			<table width="100%" class="common_table" style="margin-top: 10px;">
				<thead>
					<tr>
						<th>码户账号</th>
						<th>码户昵称</th>
				...
				<tbody class="common_tbody">
					<tr v-if="dataList.list" v-show="dataList.list.length > 0" v-for="(item, index) in dataList.list" :key="item.id">
						<td v-if="!isSelectSub">
							<span class="_info" @click="selectSub(item.id)">{{ item.cp_account }}</span>
						</td>
						<td v-if="isSelectSub">{{ item.cp_account }}</td>
	...
	export default {
		...
		methods: {
			...
			request_data() {
				...
				this.$axios.post('Code_Provider/cp_superior_check_bill', this.form).then((res) => {
					this.dataList = res;
					this.$cm_m.auth_btn()
				})
			},
			...
			selectSub(id) {
				...
				this.$cm_m.toggle_time(this);
				this.$axios.post('Code_Provider/cp_superior_check_bill', Object.assign({}, this.form, {id})).then((res) => {
					this.dataList = res;
					this.$cm_m.auth_btn()
				})
			},
