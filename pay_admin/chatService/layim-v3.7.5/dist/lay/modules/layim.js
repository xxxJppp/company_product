...
layui.define(['layer', 'laytpl', 'upload'], function(exports) {
	...
	LAYIM.prototype.setChatStatus = function(str) {
		...
		if(thatChat.data.id == serviceId) {
			str = '<span style="color:#FF5722;">在线</span>'
		}
		if(!thatChat) return;
	...
	//接受消息
	LAYIM.prototype.getMessage = function(data, userdo) {
		if(userdo) {
			getMessage(data);
			return true;
		} else {
	...
	var listTpl = function(options) {
		...
		return ['{{# var length = 0; layui.each(' + options.item + ', function(i, data){ length++; }}',
			...
			' {{ data.status === "1" ? "layim-list-gray" : "" }}"><img src="' + window.top.$Res.httpRoot +
			'/images/{{ data.avatar }}.png"><span>{{ data.alias||data.groupname||data.name||"佚名" }}</span><p>{{ data.remark||data.sign||"" }}</p><span class="layim-msg-status">new</span></li>',
			'{{# }); if(length === 0){ }}', '<li class="layim-null">' + (nodata[options.type] || "暂无数据") + '</li>',
			'{{# } }}'
		].join('');
	};

	var elemTpl = ['<div class="layui-layim-main">', '<div class="layui-layim-info">',
		'<div class="layui-layim-user">{{ d.mine.alias }}</div>', '<div class="layui-layim-status">',
		'{{# if(d.mine.status === "1"){ }}',
		'<span class="layui-icon layim-status-online" layim-event="status" lay-type="show">&#xe617;</span>',
		'{{# } else if(d.mine.status === "0") { }}',
		'<span class="layui-icon layim-status-hide" layim-event="status" lay-type="show">&#xe60f;</span>', '{{# } }}',
		'<ul class="layui-anim layim-menu-box">',
		'<li {{d.mine.status === "1" ? "class=layim-this" : ""}} layim-event="status" lay-type="online"><i class="layui-icon">&#xe618;</i><cite class="layui-icon layim-status-1">&#xe617;</cite>在线</li>',
		'<li {{d.mine.status === "0" ? "class=layim-this" : ""}} layim-event="status" lay-type="hide"><i class="layui-icon">&#xe618;</i><cite class="layui-icon layim-status-0">&#xe60f;</cite>隐身</li>',
		'</ul>', '</div>',
		...
		'<ul class="layui-layim-list {{# if(spread === "true"){ }}', ' layui-show', '{{# } }}">', listTpl({
			type: "friend",
			item: "item.list",
			index: "index"
		...
		'<li>', '<ul class="layui-layim-list layui-show layim-list-group">', listTpl({
			type: 'group'
		}), '</ul>', '</li>', '</ul>',
		'<ul class="layui-unselect layim-tab-content  {{# if(!d.base.isfriend && !d.base.isgroup){ }}layui-show{{# } }}">',
		'<li>', '<ul class="layui-layim-list layui-show layim-list-history">', listTpl({
			type: 'history'
		...
		, '<li class="layui-icon layim-tool-skin" layim-event="skin" title="更换背景" style="display:none">&#xe61b;</li>',
		'{{# if(!d.base.copyright){ }}',
	...
	//换肤模版
	var elemSkinTpl = ['<ul class="layui-layim-skin">', '{{# layui.each(d.skin, function(index, item){ }}',
	...
	var elemChatTpl = ['<div class="layim-chat layim-chat-{{d.data.type}}{{d.first ? " layui-show" : ""}}">',
		...
		'<span class="layui-icon layim-tool-image" title="上传图片" layim-event="image">&#xe60d;<input type="file" name="upload"></span><span class="layui-icon layui-icon-list layui-form-select layui-form-selected FastMsg" title="快捷回复"></span>',
		'{{# }; }}', '{{# if(d.base && d.base.uploadFile){ }}',
		'<span class="layui-icon layim-tool-image" title="发送文件" layim-event="image" data-type="file">&#xe61d;<input type="file" name="upload"></span>',
	...
	//添加好友群组模版
	var elemAddTpl = ['<div class="layim-add-box">', '<div class="layim-add-img"><img class="layui-circle" src="' +
		window.top.$Res.httpRoot + '/images/{{ d.data.avatar }}.png"><p>{{ d.data.name||"" }}</p></div>',
	...
	layui.data.content = function(content) {
		...
		content = (content || '').replace(/&(?!#?[a-zA-Z0-9]+;)/g, '&amp;')
			...
			.replace(/file\([\s\S]+?\)\[[\s\S]*?\]/g, function(str) { //转义文件
				...
				if(!href) return str;
				return '<a class="layui-layim-file" href="' + href +
					'" download target="_blank"><i class="layui-icon">&#xe61e;</i><cite>' + (text || href) + '</cite></a>';
			})
			.replace(/audio\[([^\s]+?)\]/g, function(audio) { //转义音频
				return '<div class="layui-unselect layui-layim-audio" layim-event="playAudio" data-src="' + audio.replace(
					/(^audio\[)|(\]$)/g, '') + '"><i class="layui-icon">&#xe652;</i><p>音频消息</p></div>';
			})
			.replace(/video\[([^\s]+?)\]/g, function(video) { //转义音频
				return '<div class="layui-unselect layui-layim-video" layim-event="playVideo" data-src="' + video.replace(
	...
	var layimMain, popim = function(content) {
		return layer.open({
			...
			success: function(layero) {
				var left = +layero.css('left').match(/\d/g).join('')
				layero.css('left', left - 85 + 'px');
				layimMain = layero;
	...
	var layimClose, popmin = function(content) {
		...
		return layer.open({
			...
			offset: 'rb',
			resize: false,
			content: '<img src="https://ss3.bdstatic.com/70cFv8Sh_Q1YnxGkpoWK1HF6hhy/it/u=2725122152,172062691&fm=27&gp=0.jpg"><span style="color: #cd36e6;">' +
				cache.base.title + '</span>'
	...
	var layimChat, layimMin, chatIndex, To = {},
		popchat = function(data) {
			...
			var index = chatIndex = layer.open({
				...
				anim: data.anim || 0,
				closeBtn: 1,
				content: laytpl('<ul class="layui-unselect layim-chat-list">' + elemChatList +
					'</ul><div class="layim-chat-box">' + elemChatTpl + '</div>').render(render),
				success: function(layero) {
					setTimeout(function() {
					...
					layero.find('.layim-chat-textarea').css({
						'height': '80px'
					}).find('textarea').css({
						'height': 'inherit'
					})
	...
	var setChatMin = function(newMsg) {
		...
		setChatMin.index = layer.open({
			...
			resize: false,
			area: ['182px', '50px'],
			content: '<img id="layui-layim-min" src="' + window.top.$Res.httpRoot + '/images/' + thatChat.avatar +
				'.png"><span>' + thatChat.name + '</span>',
			success: function(layero, index) {
	...
	var showOffMessage = function() {
		var thatChat = thisChat();
		var cont = $(thatChat.elem[0]);
		//	cont.find('.FastMsg').html('&#xe60a;<dl class="layui-anim layui-anim-upbit" style=" display:none">' + html + '</dl>')
		cont.find('.FastMsg').html('&#xe60a;')
		var el = cont.find('.FastMsg').find('dl');
		clearTimeout(stop);
		stop = setTimeout(function() {
			el.css('top', -(el.height() + 13) + 'px')
		}, 200)
		clearTimeout(setTime)
		//快速回复
		// cont.find('.FastMsg').hover(function () {
		// 	clearTimeout(setTime);
		// 	$(this).find('dl').show();
		...
		// })
		var style =
			'<style>.chart_sort_nums{text-align:center;width:50px}.layui-layer-page .layui-layer-content{overflow-y: auto;overflow-x: hidden;}.form_item_list:hover{background-color: rgba(204, 204, 204, 0.3);#FastMsg_layer{max-height:537px;.layui-form-label{text-align:left;}}}</style>'
		cont.find('.FastMsg').click('click', function() {
			layer.open({
				type: 1,
				id: 'FastMsg_layer',
				area: ['900px', '650px'],
				content: html + style,
				success: function(layero, index) {
					$(layero).find('.layui-colla-title').click(function() {
						setTimeout(function() {
							var height = $(layero).find('.layui-collapse').height() + 2;
							if(height > 537) {
								height = 537;
							}
							$('#FastMsg_layer').css('height', height + 'px')
						})
					})
					$('.chart_sort_content').css({
						'textAlign': 'left',
						'width': '768px',
						'whiteSpace': 'nowrap',
					...
					layui.use('element', function() {
						...
						element.init();
						$('.form_item_list').on('click', function(e) {
							var text = $.trim($(this).find('.chart_sort_content').text());
							cont.find('textarea').val(text)
							cont.find('.layim-send-btn').click();
							layer.close(index);
							e.cancelable = true;
							e.stopPropagation();
						})
	...
	var sendMessage = function() {
		if(!startWebsocket1) {
			layer.msg('当前网络链接超时，请稍后再试', {
				icon: 5
			})
			return false
		}
		var data = {
			username: cache.mine ? cache.mine.alias : '访客',
	...
	var messageNew = {},
		getMessage = function(data) {
			...
			if(thatChat.data.type + thatChat.data.id !== data.type + data.id) {
				...
				elem.find('.Red_dot').css('display', 'inline-block')
				setTimeout(function() {
	...
	var pushChatlog = function(message) {
		...
		/**
		 * hhyang 新增防止多个页面同时打开时消息重复问题
		 * 发消息的时候带一个migId 过来，参数最好为时间戳
		 */
		if(typeof isnewChat != "undefined" && isnewChat === true) {
			if(message.mine) { //是自己的发的 我们添加一个id进去
				message.msgId = +new Date();
			}
			for(item in local.chatlog) {
				var msgs = {};
				for(var i = 0; i < local.chatlog[item].length; i++) {
					var item2 = local.chatlog[item][i];
					msgs[item2.msgId] = local.chatlog[item][i]
				}
				local.chatlog[item] = Object.values(msgs)
			}
		}
		layui.data('layim', {
			key: cache.mine.id,
	...
	var chatListMore = function() {
		...
		chatMain.find('.layim-chat-system').css('display', 'none');
		var ul = chatMain.find('ul');
		var length = ul.find('li').length;

		if(length >= MAX_ITEM) {
			var first = ul.find('li').eq(0);
			if(!ul.prev().hasClass('layim-chat-system') && thatChat.data.id != serviceId) {
	...
	//表情库
	var faces = function() {
		var alt = ["[微笑]", "[嘻嘻]", "[哈哈]", "[可爱]", "[可怜]", "[挖鼻]", "[吃惊]", "[害羞]", "[挤眼]", "[闭嘴]", "[鄙视]", "[爱你]", "[泪]",
		...
		events = {
			...
			status: function(othis, e) {
					...
					if(type === 'show') {
						...
						prev.removeClass('layim-status-' + (type === '1' ? '0' : '1'))
							...
							if(val === '') {
								...
								if(data.length > 0) {
									for(var l = 0; l < data.length; l++) {
										html += '<li layim-event="chat" data-type="' + data[l].type + '" data-index="' + data[l].index +
											'" data-list="' + data[l].list + '"><img src="' + window.top.$Res.httpRoot + '/images/' + data[l].avatar +
											'.png"><span>' + (data[l].username || data[l].groupname || '佚名') + '</span><p>' + (data[l].remark || data[
												l].sign || '') + '</p></li>';
									}
								} else {
							...
							success: function(layero) {
								...
								post(members, function(res) {
									layui.each(res.list, function(index, item) {
										li += '<li data-uid="' + item.id + '"><a href="javascript:;"><img src="' + window.top.$Res.httpRoot +
											'/images/' + item.avatar + '.png"><cite>' + item.username + '</cite></a></li>';
			...
			image: function(othis) {
					...
					layui.upload.render({
						...
						accept: type,
							...
							if(typeof isnewChat != "undefined") { //改造一下他的数据格式把 新版 兼容旧版
								var newres = {
									msg: '',
									code: 0,
									data: {
								...
								if(res.status != "OK") {
									newres.code = 1;
									newres.msg = res.errorMsg;
								} else {
									newres.data.src = window.ResourcePath + res.dataList;
									newres.data.name = res.dataList;
								}
								res = newres;
							}
							if(res.code == 0) {
			...
			media: function(othis) {
					...
					layer.prompt({
						...
						shade: false,
						offset: [
							othis.offset().top - $(window).scrollTop() - 158 + 'px', othis.offset().left + 'px'
						]
					}, function(src, index) {
			...
			playVideo: function(othis) {
					...
					events.playVideo.index = layer.open({
						...
						content: '<div style="background-color: #000; height: 100%;"><video style="position: absolute; width: 100%; height: 100%;" src="' +
							videoData + '" loop="loop" autoplay="autoplay"></video></div>'
					});
				}
	...
	'modules/layim/layim.css?v=3.7.6', 'skinlayimcss'
);