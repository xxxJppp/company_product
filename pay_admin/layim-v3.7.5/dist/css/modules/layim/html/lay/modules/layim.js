...
layui.define(['layer', 'laytpl', 'upload'], function(exports) {
	...
	LAYIM.prototype.setChatStatus = function(str) {
		...
		if(thatChat.data.id == serviceId) {
			str = '<span style="color:#DA8946;">在线</span>'
		}
		if(!thatChat) return;
	...
	var layimClose, popmin = function(content) {
		...
		return layer.open({
			...
			offset: 'rb',
			resize: false,
			content: '<img src="https://ss3.bdstatic.com/70cFv8Sh_Q1YnxGkpoWK1HF6hhy/it/u=2725122152,172062691&fm=27&gp=0.jpg"><span style="color: #cd36e6;">' + cache.base.title + '</span>'
	...
	var layimChat, layimMin, chatIndex, To = {},
		popchat = function(data) {
			...
			var index = chatIndex = layer.open({
				type: 1,
				area: data.id == serviceId ? ['750px', '600px'] : "600px",
				...
				success: function(layero) {
					if(data.id == serviceId) {
						layero.find('.layim-tool-log').css('display', 'none');
						layero.find('.layim-chat-username').text(data.name)
						layero.find('.layim-chat-username cite').css('display', 'none');
						layero.find('.layim-chat-status').html('<span style="color:#DA8946;">在线</span>')
	...
	var setChatMin = function(newMsg) {
		...
		setChatMin.index = layer.open({
			...
			resize: false,
			area: ['182px', '50px'],
			content: '<img id="layui-layim-min" src="' + $Res.httpRoot + '/images/' + thatChat.avatar + '.png"><span>' + thatChat.name + '</span>',
			success: function(layero, index) {
				...
				layero.find('.layui-layer-content span').on('click', function() {
					if(cache.chat.length == 0) {
						...
						if(cache.chat[0].id == serviceId && hhyang_big) {
							$Res.tipsPublicFun({
								tips: "您当前处于上下级聊天中，是否结束当前会话？",
								icon: 1,
								btn: ['取消', '确定'],
								success: () => {
									$('.layui-layim-chat').remove();
									$('.layui-layim-min').remove();
									$('.layui-layer.layui-layer-page.layui-box.layui-layim').slideUp(100)
									hhyang_big = false;
									hhyang_brief = true;
									layer.close(index);
									newMsg ? layui.each(cache.chat, function(i, item) {
						...
						} else if(cache.chat[0].id == serviceId && hhyang_brief) {
							$Res.tipsPublicFun({
								tips: "您当前处于客服聊天中，是否结束当前会话？",
								icon: 1,
								btn: ['取消', '确定'],
								success: () => {
									$('.layui-layim-chat').remove();
							...
							if(val === '') {
								...
								if(data.length > 0) {
									for(var l = 0; l < data.length; l++) {
										html += '<li layim-event="chat" data-type="' + data[l].type + '" data-index="' + data[l].index + '" data-list="' + data[l].list + '"><img src="' + $Res.httpRoot + '/images/' + data[l].avatar + '.png"><span>' + (data[l].username || data[l].groupname || '佚名') + '</span><p>' + (data[l].remark || data[l].sign || '') + '</p></li>';
									}
								} else {
							...
							success: function(layero) {
								...
								post(members, function(res) {
									layui.each(res.list, function(index, item) {
										li += '<li data-uid="' + item.id + '"><a href="javascript:;"><img src="' + window.top.$Res.httpRoot + '/images/' + item.avatar + '.png"><cite>' + item.username + '</cite></a></li>';
