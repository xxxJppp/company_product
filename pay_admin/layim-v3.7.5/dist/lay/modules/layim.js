...
layui.define(['layer', 'laytpl', 'upload'], function(exports) {
	...
	var listTpl = function(options) {
		...
		return ['{{# var length = 0; layui.each(' + options.item + ', function(i, data){ length++; }}',
			...
			' {{ data.status === "offline" ? "layim-list-gray" : "" }}"><img src="' + window.top.$Res.httpRoot +
	...
	var elemTpl = ['<div class="layui-layim-main">', '<div class="layui-layim-info">',
		...
		'{{# if(d.mine.status === "1"){ }}',
		'<span class="layui-icon layim-status-1" layim-event="status" lay-type="show">&#xe617;</span>',
		'{{# } else if(d.mine.status === "0") { }}',
		'<span class="layui-icon layim-status-0" layim-event="status" lay-type="show">&#xe60f;</span>', '{{# } }}',
	...
	var elemChatList =
		...
		'/images/{{ d.data.avatar }}.png"><span>{{ d.data.alias||d.data.name||"佚名" }}</span>{{# if(!d.base.brief){ }}<i class="layui-icon" layim-event="closeChat">&#x1007;</i>{{# } }}</li>';
	...
	var layimChat, layimMin, chatIndex, To = {},
		popchat = function(data) {
			...
			var index = chatIndex = layer.open({
				...
				success: function(layero) {
					layero.find('.layui-unselect.layim-chat-tool').append(
						'<span class="layui-icon layui-icon-list layui-form-select layui-form-selected" title="快捷回复" id="FastMsg">&#xe60a;<dl class="layui-anim layui-anim-upbit" style=" display:none">' +
						html + '</dl></span>')

					var el = layero.find('#FastMsg').find('dl');
					...
					layero.on('click', 'dd', function(e) {
						var text = $(this).text();
						layero.find('textarea').val(text)
						$(this).parent().hide();
						layero.find('.layim-send-btn').click();
						e.cancelable = true;
						e.stopPropagation();
					})
	...
	var setChatMin = function(newMsg) {
		...
		setChatMin.index = layer.open({
			...
			success: function(layero, index) {
				var chat_Data = layui.layim.cache().chat;
				var uerId = "",
					avatarList = '',
					nameList = '';
				for(var i = 0; i < chat_Data.length; i++) {
					uerId += chat_Data[i].id + ','
					avatarList += chat_Data[i].avatar + ','
					nameList += chat_Data[i].name + ','
				}
				avatarList = avatarList.substring(0, avatarList.length - 1);
				uerId = uerId.substring(0, uerId.length - 1);
